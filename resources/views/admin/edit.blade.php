            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col mt-1">{{ __('Update ') }}{{ $admin->name }}</div>
                        @else
                        <div class="col mt-1">{{ __('Update ') }}{{ $admin->name }}</div>
                        <div class="col-auto">
                            <a href="{{ route('admin.delete', ['user_id' => $admin->id]) }}" class="btn btn-sm btn-danger"><i class="bi bi-trash3"></i></a>
                        </div>
                        @endif
                    </div>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.update', ['user_id' => $admin->id]) }}">
                        @csrf
